...
const ASSETS_TABS: ReadonlyArray<{
...
}> = [
  {
    value: "media",
    label: "Medios",
    description: "Importa video, audio e imágenes.",
  },
  {
    value: "text",
    label: "Texto",
    description: "Agrega presets de título y subtítulos.",
  },
  {
    value: "graphics",
    label: "Gráficos",
    description: "Crea formas, flechas y superposiciones SVG.",
  },
  {
    value: "effects",
    label: "Efectos",
    description: "Arrastra efectos a un clip para aplicarlos.",
  },
  {
    value: "transitions",
    label: "Transiciones",
    description: "Arrastra transiciones al borde de un clip.",
  },
  {
    value: "ai",
    label: "Generar IA",
    description: "Genera clips, subtítulos y ediciones asistidas.",
  },
  {
    value: "recipes",
    label: "Recetas",
    description: "Aplica estilos, superposiciones y textos por clip.",
  },
  {
    value: "templates",
    label: "Plantillas",
    description: "Carga diseños y presets de proyecto completo.",
  },
] as const;
    ...
    <div className="absolute inset-0 bg-black/40 backdrop-blur-[1px] flex items-center justify-center gap-2 animate-in fade-in duration-200">
      {item.kieaiError ? (
        <button
          ...
          title="Generación fallida — clic para reintentar"
        ...
        <div title="Generación KieAI en curso…" className="p-2">
        ...
        <>
          ...
          <button
            ...
            title="Agregar a la línea de tiempo"
        ...
        {isHovered && (
          <div className="flex items-center gap-1 flex-shrink-0">
            {item.kieaiError ? (
              ...
              <div className="p-1" title="Generando…">
...
const EmptyState: React.FC<{ onImport: () => void }> = ({ onImport }) => (
  <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
    ...
    <p className="text-sm text-text-secondary mb-2 font-medium">
      Sin medios importados
    </p>
    <p className="text-xs text-text-muted mb-6">
      Arrastra archivos aquí o haz clic para importar
    </p>
    <button
      onClick={onImport}
...
export const AssetsPanel: React.FC = () => {
  ...
  const setActiveTab = useCallback((tab: AssetsTab) => {
    if (activeTab === "ai" && tab !== "ai" && ttsHasUnsaved) {
      toast.warning("Audio sin guardar descartado", "Guárdalo en medios o descárgalo la próxima vez para conservarlo.");
    }
    setActiveTabRaw(tab);
    ...
    async (files: FileList | null) => {
      ...
      try {
        for (let i = 0; i < fileArray.length; i++) {
          ...
          setImportProgress(
            `Importando ${file.name} (${i + 1}/${fileArray.length})...`,
          );
          ...
          if (result.success && file.type.startsWith("video/")) {
            setImportProgress(`Extrayendo audio de ${file.name}...`);
  ...
  const handleReplaceAsset = useCallback(
    async (itemId: string) => {
      ...
      input.onchange = async (e) => {
        ...
        if (file) {
          setIsImporting(true);
          setImportProgress(`Reemplazando recurso...`);
          try {
  ...
  const handleRelinkFromFolder = useCallback(async () => {
    if (!("showDirectoryPicker" in window)) {
      toast.error("Selector de carpeta no compatible", "Revincula cada recurso faltante con su botón de actualizar.");
      return;
    }
    ...
    for (const item of placeholders) {
      ...
      if (entry) {
        setImportProgress(`Revinculando ${item.name}…`);
        try {
    ...
    if (linked > 0) {
      toast.success(`${linked} de ${placeholders.length} recurso${placeholders.length !== 1 ? "s" : ""} revinculado${placeholders.length !== 1 ? "s" : ""}`);
    } else {
      toast.error("Sin coincidencias", "Ningún archivo de la carpeta coincide por nombre con los recursos faltantes.");
    }
  }, [replaceMediaAsset]);
  ...
  const handleOpenKieAI = useCallback(async (item: MediaItem) => {
    try {
      ...
      if (!blob) {
        toast.error("Recurso no encontrado", "No se pudieron cargar los datos de imagen de este recurso.");
        return;
      }
      ...
      toast.error("Error al abrir KieAI", err instanceof Error ? err.message : "Error desconocido");
    }
  }, []);
          ...
          <div className="flex min-h-0 flex-1 flex-col border-t border-border/70">
            <div className="px-4 pt-3 pb-3 flex items-center gap-2">
              ...
              <div className="flex items-center bg-background-tertiary border border-border rounded-lg p-0.5">
                {([
                  { mode: "large" as const, icon: LayoutGrid, title: "Iconos grandes" },
                  { mode: "small" as const, icon: Grid2x2, title: "Iconos pequeños" },
                  { mode: "list" as const, icon: List, title: "Vista de lista" },
              ...
              <div className="px-4 pb-3 space-y-2">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span>Solo recursos faltantes</span>
                  </div>
                ...
                >
                  <RefreshCw size={14} />
                  <span>Revincular desde carpeta…</span>
                </button>
              </div>
            )}
                  ...
                  <div className={
                    ...
                    {mediaViewMode === "list" ? (
                      ...
                      >
                        ...
                        <span className="text-[11px] text-text-muted group-hover:text-text-secondary transition-colors font-medium">Agregar medios</span>
                      </button>
                    ) : (
                      <div className="flex flex-col">
                        <button
                        ...
                        >
                          <div className="flex flex-col items-center gap-1.5">
                            ...
                            <span className="text-[10px] text-text-muted group-hover:text-text-secondary transition-colors">Agregar medios</span>
                          </div>
                        </button>
                      </div>
                    )}
              ...
              <div className="px-4 py-4">
                  ...
                  <div className="flex gap-1.5 mb-3 flex-wrap">
                    {(["all", "solid", "gradient", "mesh", "pattern"] as const).map(
                      (cat) => (
                        ...
                        >
                          {BACKGROUND_CATEGORY_LABELS[cat] ?? cat}
                        </button>
                      ),
                    )}
                  </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      ...
                      { type: "star" as ShapeType, icon: Star, label: "Estrella" },
                      {
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {([
                      { type: "mesh-cube" as ShapeType, label: "Cubo", icon: "□" },
                      { type: "mesh-sphere" as ShapeType, label: "Esfera", icon: "○" },
                      { type: "mesh-torus" as ShapeType, label: "Toro", icon: "◯" },
                      { type: "mesh-cone" as ShapeType, label: "Cono", icon: "△" },
                      { type: "mesh-cylinder" as ShapeType, label: "Cilindro", icon: "▯" },
                      { type: "mesh-icosahedron" as ShapeType, label: "Icosaedro", icon: "◆" },
                    ]).map((mesh) => (
                      <button
                        key={mesh.type}
                        onClick={async () => {
              ...
              <div className="px-4 py-4 space-y-3">
                <button
                  onClick={async () => {
                    ...
                    if (newTextTrack) {
                      createTextClip(newTextTrack.id, 0, "Nuevo título");
                    }
                  }}
                ...
                >
                  ...
                  <p className="text-xs text-text-muted mt-1">
                    Clic para agregar texto a la línea de tiempo
                  </p>
                </button>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full bg-bg-1 relative">
        {isImporting && (
          <LoadingIndicator message={importProgress || "Importando medios..."} />
        )}
